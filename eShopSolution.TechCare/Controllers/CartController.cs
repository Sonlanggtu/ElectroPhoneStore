...
namespace eShopSolution.TechCare.Controllers
{
	...
	public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Checkout([FromBody] CheckoutViewModel request)
        {
            ...
            {
                return Ok(new { status = "fail", message = "validate", res = request.name });
			}
			...
			if (request.CartItems != null && request.CartItems.Any())
            {
                ...
                if (orderDetails.Any())
                {
					...
					result = await _orderApiClient.CreateOrder(checkoutRequest);
				}
			}
            ...
            if (result == "Failed")
            {
				return Ok(new { status = "fail", message = "Đơn đặt hàng xử lí thất bại!", res = request });
			}
			return Ok(new { status = "success", message = "Đơn đặt hàng xử lí thành công!", res = request });
